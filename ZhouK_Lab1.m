...
if arySize(1)~=arySize(2);
    ...
    [M,I] = min(abs(transpose(ary))); % Use this syntax of the min function in order to store the data for the column numbers of the largest magnitudes of each row of the matrix
    smallestMag = []; % define an empty column vector
    % iterate for each column of the matrix
    for c=1:arySize(1)
